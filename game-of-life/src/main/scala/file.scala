...
implicit val CoAdderFFunctor: Functor[CoAdderF] = new Functor[CoAdderF] {
  def map[A, B](fa: CoAdderF[A])(f: A => B): CoAdderF[B] =
    CoAdderF((x: Int) => (fa.addH(x)._1, f(fa.addH(x)._2)), f(fa.clearH), (fa.totalH._1, f(fa.totalH._2)))
    //fa.addH.map((_).map(f))
}
...
def mkCoAdder(limit: Limit, count: Count): CoAdder[(Limit, Count)] = {
  ...
  Cofree.unfoldC[CoAdderF, (Limit, Count)](limit, count)((y: (Limit, Count)) => CoAdderF(coAdd(y),coClear(y),coTotal(y)))
}
